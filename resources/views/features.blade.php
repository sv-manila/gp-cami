  ...
  <div class="wrap">
    ...
    <section>
      ...
      <div class="cards">
        ...
        <div class="card"><span class="ico">13</span><h3>Set-based backfill <span class="badge live">live</span></h3><p>The one-time backlog resolves via a handful of bulk <code>INSERT…SELECT</code> statements instead of ~26 queries per row — one command, hours not days.</p></div>
    ...
    <section>
      ...
      <div class="flowd">
        <p class="cap">gp:backfill (stage phase) — bulk chunked copy into hub-local staging, read-only source</p>
        <div class="frow">
    ...
    <section>
      ...
      <table>
        ...
        <tr><td><code>gp:backfill</code></td><td>Mode 1</td><td>The one-time backlog, end to end in a single command: stage → resolve → enrich → dedup → finalize. Set-based (bulk <code>INSERT…SELECT</code>) rather than row-by-row. Options: <code>--from-id</code>/<code>--to-id</code> (bounded / sanity run), <code>--chunk</code>.</td></tr>
      ...
      <h3 style="margin-top:22px">Backfill pipeline (one command)</h3>
      <p class="sub" style="margin-bottom:10px"><code>gp:backfill</code> runs the whole set-based pipeline. Deterministic resolution and rollups are a handful of bulk SQL statements over the staged data; dedup merges any duplicate identities (including on DEA/MMIS identifiers); finalize recomputes survivorship and rebuilds every profile.</p>
      <div class="pipe">
        <div class="step"><div class="k">STAGE</div><h4>Bulk load</h4><p>Employees + source tables copied into hub-local staging in batches.</p></div>
        <div class="step"><div class="k">RESOLVE</div><h4>Tier SQL</h4><p>One create+link pair per deterministic key tier, set-based over the whole batch.</p></div>
        <div class="step"><div class="k">ENRICH + DEDUP</div><h4>Consolidate</h4><p>Licenses, addresses and identifiers populated; identities sharing a key merged.</p></div>
        <div class="step"><div class="k">FINALIZE</div><h4>Materialize</h4><p>Survivorship winners + the denormalized profile per identity.</p></div>
      </div>
    </section>
